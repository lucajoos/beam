const Entry = ({ name, icons=[null, null], onClick=()=>{}, onClickIcon=()=>{}, isEnabled=true, className='' }) => {
    return (
        <div className={`w-[420px] max-w-[calc(100vw-2rem)] cursor-pointer flex justify-between gap-8 items-center bg-gray-200 rounded${className.length > 0 ? ` ${className}` : ''}`} onClick={() => isEnabled && onClick()}>
